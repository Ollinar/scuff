...
func vipsgenGifsaveWithOptions(in *C.VipsImage, filename string, dither float64, effort int, bitdepth int, interframeMaxerror float64, reuse bool, interpaletteMaxerror float64, interlace bool, keep Keep, background []float64, pageHeight int, profile string) (error) {
	...
	cprofile := C.CString(profile)
	defer freeCString(cprofile)
	if err := C.vipsgen_gifsave_with_options(in, cfilename, C.double(dither), C.int(effort), C.int(bitdepth), C.double(interframeMaxerror), C.int(boolToInt(reuse)), C.double(interpaletteMaxerror), C.int(boolToInt(interlace)), C.VipsForeignKeep(keep), cbackground, cbackgroundLength, C.int(pageHeight), cprofile); err != 0 {
		return handleVipsError()
	}
	return nil
}
...
func vipsgenGifsaveBufferWithOptions(in *C.VipsImage, dither float64, effort int, bitdepth int, interframeMaxerror float64, reuse bool, interpaletteMaxerror float64, interlace bool, keep Keep, background []float64, pageHeight int, profile string) ([]byte, error) {
	...
	cprofile := C.CString(profile)
	defer freeCString(cprofile)
	if err := C.vipsgen_gifsave_buffer_with_options(in, &buf, &length, C.double(dither), C.int(effort), C.int(bitdepth), C.double(interframeMaxerror), C.int(boolToInt(reuse)), C.double(interpaletteMaxerror), C.int(boolToInt(interlace)), C.VipsForeignKeep(keep), cbackground, cbackgroundLength, C.int(pageHeight), cprofile); err != 0 {
		return nil, handleVipsError()
	}
	return bufferToBytes(buf, length), nil
}
...
func vipsgenGifsaveTargetWithOptions(in *C.VipsImage, target *C.VipsTargetCustom, dither float64, effort int, bitdepth int, interframeMaxerror float64, reuse bool, interpaletteMaxerror float64, interlace bool, keep Keep, background []float64, pageHeight int, profile string) (error) {
	...
	cprofile := C.CString(profile)
	defer freeCString(cprofile)
	if err := C.vipsgen_gifsave_target_with_options(in, target, C.double(dither), C.int(effort), C.int(bitdepth), C.double(interframeMaxerror), C.int(boolToInt(reuse)), C.double(interpaletteMaxerror), C.int(boolToInt(interlace)), C.VipsForeignKeep(keep), cbackground, cbackgroundLength, C.int(pageHeight), cprofile); err != 0 {
		return handleVipsError()
	}
	return nil
}
